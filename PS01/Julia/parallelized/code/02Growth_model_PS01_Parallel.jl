...
@everywhere mutable struct Results
    val_func::SharedArray{Float64,2} #value function: 2-dim b/c value fn differs by productivity state
    pol_func::SharedArray{Float64,2} #policy function: 2-dim b/c value fn differs by productivity state
end

#function for initializing model primitives and results
@everywhere function Initialize()
    prim = Primitives() #initialize primitives (creates object of type "Primitives")
    val_func = SharedArray{Float64}(zeros(prim.nk, prim.nz)) #initial value function guess (nk x nz)
    pol_func = SharedArray{Float64}(zeros(prim.nk, prim.nz)) #initial policy function guess (nk x nz)
...
@everywhere function Bellman(prim::Primitives,res::Results)
    ...
    @sync @distributed for i = 1:nz*nk #iterating over values of z and k and solving for optimal k'(k,z)
        z_index = mod(i,nz) + 1
