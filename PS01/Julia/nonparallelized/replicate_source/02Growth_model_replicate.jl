...
function Bellman(prim::Primitives,res::Results)
    ...
    @unpack k_grid, β, δ, θ, nk = prim #unpack model primitives
            ...
            if c>0 #check for positivity
                ...
                if val>candidate_max #check for new max value
                    ...
                    choice_lower = kp_index #update lowest possible choice
                        #= k'(k) should be monotone. So when we find k'(k1), and then want to
                        solve for k'(k2) where k2>k1, we no longer need to check optimizers
                        k' such that k' < k'(k1). In other words, this step makes
                        the algorithm more efficient by skipping redundant guesses for k'. =#
                end
            end
        end
